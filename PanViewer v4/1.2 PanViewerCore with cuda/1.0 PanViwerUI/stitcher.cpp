...
void DetectAndDescribe(cv::Mat& image, std::vector<cv::KeyPoint>& OutputKeypoints, cv::Mat& OutputDescriptor) {
...
void matchKeypoints(cv::Mat& imageA, cv::Mat& imageB, std::vector<cv::KeyPoint>& keypointA, std::vector<cv::KeyPoint>& keypointB, cv::Mat& featuresA, cv::Mat& featuresB, float ratio, double repojThresh, std::vector<cv::DMatch>& OutputGoodMatches, cv::Mat& OutputHomographyM) {
...
void getHomographyMatrix(cv::Mat& imageA, cv::Mat& imageB, float ratio, double repojThresh, cv::Mat& outHomographyM) {
...
void stitchFrames(cv::Mat& imageA, cv::Mat& imageB, cv::Mat& inputHomography, cv::Mat& finalImage) {
	cv::Mat result;
...
void stitcher::calibrate(cv::Mat& left, cv::Mat& middle, cv::Mat& right) {
	...
	stitchFrames(transL, transM, homographyLM, transResult);
	cv::flip(transResult, result, 1);
	getHomographyMatrix(right, result, 0.75, 4.0, homographyMR);
}

void stitcher::stitch(cv::Mat& left, cv::Mat& middle, cv::Mat& right, cv::Mat& OutputImage) {
	cv::Mat transL, transM, transResult, result;
	cv::flip(left, transL, 1);
	cv::flip(middle, transM, 1);
	...
	cv::flip(transResult, result, 1);
	stitchFrames(right, result, homographyMR, OutputImage);
}
