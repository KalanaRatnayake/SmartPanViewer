...
void stitch(Mat& imageA, Mat& imageB, float ratio, double repojThresh, bool showMatches, Mat& finalImage, Mat& visiblelines) {
	...
	finalImage = Mat(Size(result.cols, imageB.rows), CV_8UC3);
...
int main() {
	...
	Mat imageM = cv::imread("middle.jpg");
	Mat imageR = cv::imread("right.jpg");


	if (imageL.empty()) std::cerr << "Could not open imageL file" << std::endl;
	if (imageM.empty()) std::cerr << "Could not open imageM file" << std::endl;
	if (imageR.empty()) std::cerr << "Could not open imageR file" << std::endl;

	//flip image to stitch left with middle first
	flip(imageL, transL, 1);
	flip(imageM, transM, 1);
	stitch(transL, transM, 0.75, 4.0, true, transResult, visiblelines);

	flip(transResult, result, 1);
	stitch(imageR, result, 0.75, 4.0, true, result2, visiblelines2);

	if (result2.empty()) std::cerr << "Could not load result" << std::endl;
